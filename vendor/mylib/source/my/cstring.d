...
auto tempCString(To = char, From)(scope From str)
    ...
    static if (To.sizeof == CF.sizeof && is(typeof(res._buff[0 .. str.length] = str[]))) {
        if (str.length < res._buff.length) {
            ...
            import my.internal.memory : enforceMalloc;
        ...
        static assert(!(isSomeString!From && CF.sizeof == To.sizeof),
                "Should be using slice assignment.");
        To[] p = res._buff;
        size_t i;
...
private:
    ...
    version (StdUnittest) // the 'small string optimization'
    {
...
private To[] trustedRealloc(To)(scope To[] buf, size_t strLength, bool bufIsOnStack) @trusted @nogc pure nothrow {
    pragma(inline, false); // because it's rarely called

    import my.internal.memory : enforceMalloc, enforceRealloc;
