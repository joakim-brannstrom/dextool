...
private:
    ...
    union Storage {
        ...
        static foreach (i, T; Types) {
            @trusted this(inout(T) val) inout {
                values[i] = val;
            }
        }
    }
    ...
    @trusted ref inout(T) trustedGet(T)() inout {
...
private template FunctionOverloads(alias fun) if (isFunction!fun) {
    ...
    alias FunctionOverloads = AliasSeq!(__traits(getOverloads, __traits(parent,
            fun), __traits(identifier, fun)));
}

// A handler with an opCall overload for each overload of fun
private template overloadHandler(alias fun) if (isFunction!fun) {
    ...
    auto matchImpl(Self)(auto ref Self self)
        ...
        static foreach (hid, handler; allHandlers) {
            static assert(handlerIndices[].canFind(hid), "handler `" ~ __traits(identifier,
                    handler) ~ "` " ~ "of type `" ~ typeof(handler).stringof ~ "` "
                    ~ "never matches");
        }
    }
}
...
unittest {
    ...
    assert(!__traits(compiles, () @safe {
            x.match!((ref int n) => &n, _ => null,);
        }));

    assert(__traits(compiles, () @system {
            return x.match!((ref int n) => &n, _ => null);
        }));
}
