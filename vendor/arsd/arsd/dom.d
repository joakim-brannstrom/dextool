	...
	*/
    ...
    Utf8Stream handleDataEncoding()(in string rawdata, string dataEncoding, bool strict) {
        ...
        if (dataEncoding is null) {
            ...
            else // if we really don't know by here, it means we already tried UTF-8,
    ...
    private Utf8Stream toUtf8Stream(in string rawdata) {
        string data = rawdata;
	...
	*/
    ...
    void parseStream(Utf8Stream data, bool caseSensitive = false, bool strict = false) {
        ...
        void parseError(string message) {
            throw new MarkupException(format("char %d (line %d): %s", pos,
                    getLineNumber(pos), message));
        }
                ...
                if (data[pos] == '<') {
                    if (strict)
                        throw new MarkupException(
                                "The character < can never appear in an attribute name. Line " ~ to!string(
                                getLineNumber(pos)));
                    else
        ...
        string readAttributeValue() {
            ...
            case '"':
                ...
                if (strict && pos == data.length)
                    throw new MarkupException(
                            "Unclosed attribute value, started on char " ~ to!string(started));
            ...
            default:
                ...
                auto start = pos;
                while (pos < data.length && data[pos] != '>' && // unquoted attributes might be urls, so gotta be careful with them and self-closed elements
                        !(data[pos] == '/'
        ...
        Ele readElement(string[] parentChain = null) {
            ...
            if (pos >= data.length) {
                if (strict) {
                    throw new MarkupException(
                            "Gone over the input (is there no root element or did it never close?), chain: " ~ to!string(
                            parentChain));
                } else {
            ...
            default:
                ...
                Ele addTag(bool selfClosed) {
                    if (selfClosed)
                        ...
                        if (!strict)
                            if (tagName.isInArray(selfClosedElements)) // these are de-facto self closed
                                selfClosed = true;
                    }
                    ...
                    while (!closed) {
                        ...
                        if (n.type == 4)
                            return n; // the document is empty
                        ...
                        if (n.type == 3 && n.element !is null) {
                            ...
                            if (n.payload != tagName) {
                                if (strict)
                                    parseError(format("mismatched tag: </%s> != <%s> (opened on line %d)", n.payload,
                                            tagName, getLineNumber(whereThisTagStarted)));
                                else {
                                    ...
                                    if (!found) // if not found in the tree though, it's probably just text
                                        e.appendChild(TextNode.fromUndecodedString(this,
                                                "</" ~ n.payload ~ ">"));
                                }
                            } else {
                                if (n.element) {
                ...
                moreAttributes:
                    ...
                    default: // it is an attribute
                        ...
                        if (strict && ateAny)
                            throw new MarkupException(
                                    "inappropriate whitespace after attribute name");
                        ...
                        if (data[pos] == '=') {
                            ...
                            if (strict && ateAny)
                                throw new MarkupException(
                                        "inappropriate whitespace after attribute equals");
                    ...
                    blankValue:
                        ...
                        if (attrName.strip().length)
                            ...
                            throw new MarkupException("wtf, zero length attribute name");
...
class Element {
    ...
    void addChildren(T...)(T t) {
        foreach (item; t) {
            static if (is(item : Element))
                ...
                static assert(0, "Cannot pass " ~ typeof(item).stringof ~ " to addChildren");
        }
    }
	...
	*/
    ...
    private void sendObserverEvent(DomMutationOperations operation,
            string s1 = null, string s2 = null, Element r = null, Element r2 = null) {
    ...
    static Element make(string tagName, string childInfo = null, string childInfo2 = null) {
        ...
        if (childInfo !is null) switch (tagName) {
    ...
    this(Document _parentDocument, string _tagName,
            string[string] _attributes = null, bool _selfClosed = false) {
	...
	*/
    @property string opDispatch(string name)(string v = null)
            if (!isConvenientAttribute(name)) {
        static assert(0,
                "Don't use " ~ name ~ " direct on Element, instead use element.attrs.attributeName");
    }
...
public:
        ...
        if (position is null)
            ...
            foreach (i, child; children) {
                if (child is position) {
                    children = children[0 .. i] ~ e.children ~ children[i .. $];
                    break;
                }
            }
        }
	...
	*/
    ...
    Element addField(string label, string name, string type = "text",
            FormFieldOptions fieldOptions = FormFieldOptions.none) {
        auto fs = this;
        ...
        Element input;
        if (type == "textarea")
            input = i.addChild("textarea").setAttribute("name", name).setAttribute("rows", "6");
        else
            input = i.addChild("input").setAttribute("name", name).setAttribute("type", type);
    ...
    Element addField(Element label, string name, string type = "text",
            FormFieldOptions fieldOptions = FormFieldOptions.none) {
        auto fs = this;
        auto i = fs.addChild("label");
        i.addChild(label);
...
string htmlEntitiesDecode(string data, bool strict = false) {
    ...
    foreach (dchar ch; data) {
        if (tryingEntity) {
            ...
            if (ch == '&') {
                if (strict)
                    throw new Exception("unterminated entity; & inside another at " ~ to!string(
                            entityBeingTried[0 .. entityBeingTriedLength]));
                ...
                // e.g. you &amp i
                if (strict)
                    throw new Exception("unterminated entity at " ~ to!string(
                            entityBeingTried[0 .. entityBeingTriedLength]));
                else {
                    tryingEntity = false;
...
public:
    ...
    string opDispatch(string name)(string v = null) if (0) {
        return null;
    ...
    override string toPrettyString(bool insertComments = false,
            int indentationLevel = 0, string indentWith = "\t") const {
        string s;
...
class Form : Element {
    ...
    override Element addField(string label, string name, string type = "text",
            FormFieldOptions fieldOptions = FormFieldOptions.none) {
    ...
    override Element addField(string label, string name, string[string] options,
            FormFieldOptions fieldOptions = FormFieldOptions.none) {
        auto t = this.querySelector("fieldset div");
        if (t is null)
    ...
    void setValue(string field, string value, bool makeNew) {
        ...
        if (eles.length == 1) {
            ...
            case "select":
                ...
                if (!found) {
                    e.addChild("option", value).setAttribute("selected", "selected");
                }
                break;
            }
        } else {
    ...
    string getPostableData() {
        ...
        foreach (e; getElementsBySelector("[name]")) {
            ...
            ret ~= std.uri.encodeComponent(e.name) ~ "=" ~ std.uri.encodeComponent(
                    getValue(e.name));
...
class Table : Element {
    ...
    in {
        if (tablePortition is null)
            ...
            assert(tablePortition.tagName == "tbody"
                    || tablePortition.tagName == "tfoot" || tablePortition.tagName == "thead");
        }
    }
    body {
        if (tablePortition is null)
...
int intFromHex(string hex) {
    ...
    for (sizediff_t a = hex.length - 1; a >= 0; a--) {
        ...
        if (q >= '0' && q <= '9')
            ...
            throw new Exception("Illegal hex character: " ~ q);
...
string[] lexSelector(string selstr) {
        ...
        foreach (sizediff_t tidx, immutable token; selectorTokens) {
            if (c == token[0]) {
                if (token.length > 1) {
                    ...
                    if (str.length - stpos < 2 || str[stpos + 1] != token[1])
                        continue;
                }
                return tidx;
            }
    ...
    static bool isBlankAt()(string str, size_t pos) {
        ...
        return (pos < str.length && // in string
                (str[pos] <= 32 || // space
    ...
    while ((selstr = removeLeadingBlanks(selstr)).length > 0) {
        ...
        while (curpos < selstr.length) {
            if (selstr[curpos] == '\\') {
                ...
                if (isBlankAt(selstr, curpos) || idToken(selstr, curpos) >= 0)
                    break;
                ++curpos;
            }
        }
...
struct SelectorPart {
    ...
    string toString() {
        ...
        foreach (a; nthChild)
            ret ~= ":nth-child(" ~ a.toString ~ ")";
        foreach (a; nthOfType)
            ret ~= ":nth-of-type(" ~ a.toString ~ ")";
        foreach (a; nthLastOfType)
            ret ~= ":nth-last-of-type(" ~ a.toString ~ ")";
    ...
    bool matchElement(Element e) {
        ...
        foreach (a; attributesEqual)
            if (a[0]!in e.attributes || e.attributes[a[0]] != a[1])
                return false;
        foreach (a; attributesNotEqual) // FIXME: maybe it should say null counts... this just bit me.
...
struct ParsedNth {
    ...
    string toString() {
        return format("%dn%s%d%s%s", multiplier, adder >= 0 ? "+" : "", adder,
                of.length ? " of " : "", of);
    }
...
SelectorComponent[] parseSelectorString(string selector, bool caseSensitiveTags = true) {
    ...
    while (tokens.length > 0) {
        ...
        if (end > 0)
            ret ~= parseSelector(tokens[0 .. end], caseSensitiveTags);
        if (tokens.length - end < 2)
            break;
        tokens = tokens[end + 1 .. $];
    }
    return ret;
...
class CssStyle {
    ...
    this(string rule, string content) {
        ...
        foreach (part; content.split(";")) {
            ...
            p.value = part[idx + 1 .. $].replace("! important", "!important")
                .replace("!important", "").strip(); // FIXME don't drop important
 ...
 <table>
  ...
  <tr>
   ...
   <td><a href="/reviews/8831">[Show]</a></td>
    ...
    auto rd = doc.root.querySelector(
            `div.roundedbox > table > caption.boxheader + tr + tr + tr > td > a[href^=/reviews/]`);
    assert(rd !is null);
    assert(rd.href == "/reviews/8832");

    rd = doc.querySelector(
            `div.roundedbox > table > caption.boxheader + tr + tr + tr > td > a[href^=/reviews/]`);
    assert(rd !is null);
    assert(rd.href == "/reviews/8832");
}
