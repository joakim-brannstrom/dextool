...
auto makeTestCaseSimilarityAnalyse(ref Database db, ref const ConfigReport conf,
    ...
    doc.root.childElements("head")[0].addChild("script")
        .addChild(new RawSource(doc, jsTableOnClick));

    //toHtml(db, doc, reportTestCaseSimilarityAnalyse(db, kinds, 5), doc.mainBody);
...
//void toHtml(ref Database db, Document doc, TestCaseSimilarityAnalyse result, Element root) {
